...
### 你只需要把你要请求的网址构造成框架规定的格式，然后放入到工作队列中即可， 其他的请求的相关处理， 线程池都已经封装好了。
### 当请求之后，若成功框架会返回请求的内容与请求的网址， 当请求由于出现异常时，会有错误日志出现。
### 解析部分还是按需选择吧，提供了re， lxml和 json的解析接口，但是具体的解析路径如xpath，pattern还是需要自己写的
### 若想保存数据，默认提供保存到mongodb中。
